...
- มี 9 ด่าน ด่านละ 3 ห้อง รวมบอส; ทุกตัวให้ EXP/ทอง/ของดรอป พร้อม floating tab 2.8 วินาที ของซ้ำเปลี่ยนเป็น 30 gold จบด่านได้โบนัสทองและสรุปรวม
...
- อุปกรณ์ 10 ช่อง: หมวก เกราะ กางเกง รองเท้า อาวุธหลัก อาวุธรอง แหวน 2 สร้อย ผ้าคลุม รวมของ 59 ชิ้น; กดช่องแล้วเลือกจาก inventory เพื่อสวม/ถอด
...
`npm test` ตรวจทิศและจังหวะป้องกัน, gestures, เงิน/อุปกรณ์, save ที่เสียหาย และ game loop จริงผ่านทั้ง 9 ด่าน รวม stun, spell cooldown, ward, pause และ retry หลังตาย มี simulation 60 fps ใช้อุปกรณ์เริ่มต้นและอินพุตป้องกันตามเวลาของ scheduler จริงจนจบทุกด่าน รวมตรวจ daily rollover และการรับรางวัลซ้ำ
...
## Tempest / Tide / Eclipse expansion

เพิ่มด่าน 7–9 ต่อจาก Obsidian Inferno ปลดล็อกทีละด่านตามระบบเดิม เซฟที่จบ 6 ด่านแล้วให้เล่นด่าน 6 จบอีกครั้งเพื่อปลดล็อกด่าน 7

| ด่าน | ศัตรูใหม่ | รูปแบบต่อสู้ | ของดรอปตามห้อง |
| --- | --- | --- | --- |
| Tempest Spire | Tempest Lancer | สลับ normal/heavy ทิศขึ้น → ลง → ซ้าย → ขวา; ฝนและสายฟ้า | Thunderstep, Tempest Crown, Stormpiercer |
| The Drowned Sanctum | Drowned Castellan | heavy → normal → sweep → normal; น้ำและสาหร่าย | Undertow, Abyssal Mail, Pearl Bastion |
| Eclipse Citadel | Eclipse Oracle | normal → sweep → normal → heavy → normal; วงคราสและละอองดาว | Duskweave Leggings, Veil of Midnight, Eventide Scepter |

- ศัตรูใหม่เป็น variants ใช้ atlas ของ frost/demon/spider ปรับสี โดยคงเฟรมโจมตีและ guard เดิม ไม่ใช่ sprite วาดใหม่ แพตเทิร์นเริ่มใหม่ทุกห้อง
- Equipment ใหม่ 12 ชิ้น ซื้อได้ใน Forge ตามระบบเดิม: สายพายุเน้นโจมตี, สายน้ำเน้น HP/DEF และโล่, สายคราสมี staff สองมือ/มีดรอง/เกราะเบา
- Eye of the Storm, Heart of the Deep และ Moonless Talon ซื้อใน Forge; อีก 9 ชิ้นได้ทั้งซื้อและดรอป
- ไอคอนและอาวุธใช้ atlas เดิมปรับสี ไม่มี passive ธาตุหรือ set bonus; ค่าสถานะจริงแสดงใน inventory
- เอฟเฟกต์ฉากเป็นภาพตกแต่ง ไม่มีดาเมจแฝง และใช้กติกา parry/dodge/block เดิม

Validation: `npm test` ครอบคลุมทั้ง 9 ด่านด้วย scheduler จริง รวม starter gear แบบป้องกันถูกจังหวะ, loot, progression, อุปกรณ์ใหม่ทั้ง 12 ชิ้น และเซฟเดิมด่าน 6 การทดสอบนี้ใช้ Canvas/DOM mocks ยังต้องตรวจภาพและสัมผัสบนมือถือจริง
